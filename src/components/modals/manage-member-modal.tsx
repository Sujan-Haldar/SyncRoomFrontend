...
export const ManageMemberModal = () => {
  ...
  const { server, updateMembers } = useServerStore();
    ...
    updateMemberApi({
      ...
      .then((res) => {
        if (res?.data?.data && server) {
          updateMembers([
            ...server.members.filter((member) => member.id !== id),
            res?.data?.data,
          ]);
        }
      })
  ...
  const removeMemberFromServer = (id: string) => {
    ...
    deleteMemberApi({
      ...
      .then((res) => {
        if (res && server) {
          updateMembers(server.members.filter((member) => member.id !== id));
        }
      })
      .catch((err) => {
        console.log(err);
        ...
        <ScrollArea className="mt-8 max-h-[420px] pr-6">
          {server?.members.map((member) => (
            <div key={member.id} className="flex items-center gap-x-2 mb-6">
              ...
              {server.user.id !== member.user.id && loadingId !== member.id && (
